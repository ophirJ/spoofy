import { DataGridPro, GridRowParams, GridRowSelectionModel, LicenseInfo } from '@mui/x-data-grid-pro';
...
const GenericTable: React.FC<props> = ({ songs }) => {
    ...
    const selectionModel = useAppSelector(state => state.playingSong.selectionModel);
    ...
    const selectSong = (params: GridRowParams<any>) => {
        let song;
        playingSong?.id == params.id ? song = undefined : song = songs.find(song => song.id == params.id);
        dispatch(setSong(song));
    }

    const rows: GridRowsProp[] = [];
    songs.map(song => rows.push({
        id: song.id,
        song: song.name,
    ...
    return (
        <div className={classes.tableContainer}>
            <DataGridPro
                ...
                onRowSelectionModelChange={(newSelectionModel) => changeSelectionMode(newSelectionModel)}
